package com.maheswara660.filora.screen.main.tab.files
...
import com.maheswara660.filora.common.getIndexIf
import com.maheswara660.filora.common.getMimeType
import com.maheswara660.filora.common.isNot
import com.maheswara660.filora.common.orIf
import com.maheswara660.filora.common.removeIf
import com.maheswara660.filora.screen.main.MainActivity
...
import com.maheswara660.filora.screen.main.tab.files.misc.FileListCategory
import com.maheswara660.filora.screen.main.tab.files.misc.FileMimeType
import com.maheswara660.filora.screen.main.tab.files.misc.FileMimeType.anyFileType
...
import com.maheswara660.filora.screen.main.tab.files.state.BottomOptionsBarState
import com.maheswara660.filora.screen.main.tab.files.state.DialogsState
import com.maheswara660.filora.screen.main.tab.files.task.CompressTask
...
class FilesTab(
    ...
    private suspend fun createTitle(): String {
        if (activeFolder.uniquePath == globalClass.recycleBinDir.uniquePath) {
            return globalClass.getString(R.string.recycle_bin)
        }

        if (activeFolder.uniquePath == Environment.getExternalStorageDirectory().absolutePath) {
            return globalClass.getString(R.string.internal_storage)
        }

        if (activeFolder is VirtualFileHolder) {
            return activeFolder.displayName
        }
    ...
    override fun onBackPressed(): Boolean {
        if (unselectAnySelectedFiles()) {
            return true
        } else if (activeFolder.uniquePath == globalClass.recycleBinDir.uniquePath) {
            // If in Recycle Bin root, go back to Home tab
    ...
    fun selectAll() {
        activeFolderContent.forEach {
            selectedFiles[it.uniquePath] = it
        }
        onSelectionChange()
        quickReloadFiles()
    }

    fun openFile(context: Context, item: ContentHolder, options: Bundle? = null) {
        if (item is LocalFileHolder && item.isApk()) {
            toggleApkDialog(item)
        } else {
            // If it's an audio file, prepare the playlist
            if (FileMimeType.audioFileType.contains(item.extension)) {
                globalClass.pendingAudioPlaylist = activeFolderContent
                    .filter { it is LocalFileHolder && FileMimeType.audioFileType.contains(it.extension) }
                    .map { (it as LocalFileHolder).file }
            }
    ...
    suspend fun openFolderImpl(
        ...
        listFiles { newContent -> // Main thread
            ...
            if (activeFolder is VirtualFileHolder) {
                val newCategories = (activeFolder as VirtualFileHolder).getCategories()
                if (categories.size != newCategories.size || !categories.containsAll(newCategories)) {
                    categories.clear()
