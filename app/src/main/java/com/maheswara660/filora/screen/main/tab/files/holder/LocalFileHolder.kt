...
import com.maheswara660.filora.common.fromJson
import com.maheswara660.filora.common.hasParent
...
import com.maheswara660.filora.screen.main.tab.files.misc.ContentCount
import com.maheswara660.filora.screen.main.tab.files.misc.DefaultOpeningMethods
import com.maheswara660.filora.screen.main.tab.files.misc.AppInstaller
...
import com.maheswara660.filora.screen.viewer.audio.AudioPlayerActivity
import com.maheswara660.filora.screen.viewer.image.ImageViewerActivity
import com.maheswara660.filora.screen.viewer.pdf.PdfViewerActivity
import com.maheswara660.filora.screen.viewer.video.VideoPlayerActivity
...
class LocalFileHolder(val file: File) : ContentHolder() {
    ...
    override fun open(
        ...
        defaultOpeningMethods.openingMethods.forEach {
            if (it.extension == extension) {
                openFileWithPackage(context, it.packageName, it.className, options)
                return
            }
        }

        if (handleSupportedFiles(skipSupportedExtensions, context, options)) {
            return
        }
        ...
        Intent(Intent.ACTION_VIEW).let { newIntent ->
            ...
            try {
                ...
                if (!anonymous) {
                    open(context, anonymous = true, skipSupportedExtensions = true, null, options)
                } else {
    ...
    fun openFileWithPackage(context: Context, packageName: String, className: String, options: Bundle? = null) {
        val uri = createUri()
    ...
    private fun handleSupportedFiles(skipSupportedExtensions: Boolean, context: Context, options: Bundle? = null): Boolean {
        if (filoraPrefsFileType == extension) {
        ...
        if (FileMimeType.supportedArchiveFileType.contains(extension)) {
            if (isApk()) {
                ...
                AppInstaller.installApk(context, file)
                return true
            }
        ...
        if (isApkBundle()) {
            if (skipSupportedExtensions) return false
            runBlocking { AppInstaller.installSplitApk(context, file) }
            return true
        }
